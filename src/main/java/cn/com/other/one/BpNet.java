...
public class BpNet {
     ...
     */
    public BpNet(int inPutNum,int outPutNum,double lr, int hidden) {
        this.lr = lr;
        //设置权值矩阵的行和列数
        this.inputHiddenWeight = new double[inPutNum][hidden];
        this.hiddenOutputWeight = new double[hidden][outPutNum];
        //初始化前一次的训练误差平方和为0
     ...
     */
    private void calculateError(double[] trainOutput,double[] curOutput){

        try {
            curOutError = Matrix.matrixSub(trainOutput,curOutput );
     ...
     */
    public void train(double[][] trainInput,double[] trainOutput)throws MatrixException{
        //
        ...
        o = outOut;
        //计算当前训练的误差（输出层的输出误差）
        calculateError(trainOutput,outOut);
        //修正学习速率
        updateLr();
        //输出层的输入误差
     ...
     * @param num 训练的次数。
     */
    public void train(double[][] trainInput,double[] output,int num)throws MatrixException{
        for (int i=1;i<=num;i++){
            train(trainInput,output);
            if (i%100 == 0){
